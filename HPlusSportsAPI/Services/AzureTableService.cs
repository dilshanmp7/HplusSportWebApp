...
namespace HPlusSportsAPI.Services
{
    ...
    public class AzureTableService : ITableService
    {
        ...
        {
            CloudStorageAccount account =CloudStorageAccount.Parse(config[Constants.KEY_STORAGE_CNN]);
            var tableClient = account.CreateCloudTableClient();
            var table = tableClient.GetTableReference(tableName);
            await table.CreateIfNotExistsAsync();
            var historyQuery = new TableQuery<OrderHistoryItem>().Where(TableQuery.GenerateFilterCondition("PartitionKey",
